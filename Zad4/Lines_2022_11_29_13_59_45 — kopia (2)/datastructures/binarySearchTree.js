...
class BinarySearchTree {
   ...
   */
  lowerBound(value, includeEqual = true, x=-1) {
    let lowerBound = null;

    const lowerBoundRecursive = (current, x) => {
      if (current === null) return lowerBound;

      const compare = this._compare(value, current.getValue(), x);
      if (compare > 0 || (includeEqual && compare === 0)) {
        if (lowerBound === null || this._compare(lowerBound.getValue(), current.getValue(), x) <= 0) {
          lowerBound = current;
        }
        return lowerBoundRecursive(current.getRight(), x);
      }

      return lowerBoundRecursive(current.getLeft(), x);
    };

    return lowerBoundRecursive(this._root, x);
  }
   ...
   * @return {BinarySearchTreeNode|null}
   */
  floor(value, includeEqual = true, x=-1) {
    return this.lowerBound(value, includeEqual, x);
  }
   ...
   * @return {BinarySearchTreeNode|null}
   */
  upperBound(value, includeEqual = true, x=-1) {
    let upperBound = null;

    const upperBoundRecursive = (current, x) => {
      if (current === null) return upperBound;

      const compare = this._compare(value, current.getValue(), x);
      if (compare < 0 || (includeEqual && compare === 0)) {
        if (upperBound === null || this._compare(upperBound.getValue(), current.getValue(), x) >= 0) {
          upperBound = current;
        }
        return upperBoundRecursive(current.getLeft(), x);
      }

      return upperBoundRecursive(current.getRight(), x);
    };

    return upperBoundRecursive(this._root, x);
  }
   ...
   * @return {BinarySearchTreeNode|null}
   */
  ceil(value, includeEqual = true, x=-1) {
    return this.upperBound(value, includeEqual, x);
  }
  ...
  remove(value, x=-1) {
    const removeRecursively = (val, current, x) => {
      ...
      if (compare < 0) return removeRecursively(val, current.getLeft(), x);
      if (compare > 0) return removeRecursively(val, current.getRight(), x);
      ...
      return removeRecursively(minRight.getValue(), minRight, x);
    };
